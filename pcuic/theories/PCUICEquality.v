...
Definition cmp_instance (cmp_qual : Quality.t -> Quality.t -> Prop) (cmp_univ : Universe.t -> Universe.t -> Prop)
  ...
  fun u u' =>
    Forall2 cmp_qual (Instance.qualities u) (Instance.qualities u') /\
...
Definition cmp_opt_variance cmp_qual cmp_univ pb v :=
  ...
  | Variance v => fun u u' => cmp_instance (cmp_qual Conv) (cmp_univ Conv) u u' \/ cmp_universe_instance_variance cmp_univ pb v (Instance.universes u) (Instance.universes u')
  end.
...
Lemma cmp_opt_variance_var_dec cmp_qual cmp_univ pb v ui ui' :
  RelationClasses.subrelation (cmp_univ Conv) (cmp_univ pb) ->
  cmp_instance (cmp_qual Conv) (cmp_univ Conv) ui ui' \/ cmp_universe_instance_variance cmp_univ pb v (Instance.universes ui) (Instance.universes ui') ->
  { cmp_instance (cmp_qual Conv) (cmp_univ Conv) ui ui' } + { cmp_universe_instance_variance cmp_univ pb v (Instance.universes ui) (Instance.universes ui') }.
Proof.
  intros subr H.
...
Lemma cmp_opt_variance_or_impl cmp_qual cmp_universe1 cmp_universe2 cmp_universe3 pb1 pb2 pb3 v u1 u1' u2 u2' u3 u3' :
  ...
  (cmp_instance (cmp_qual Conv) (cmp_universe1 Conv) u1 u1' -> cmp_instance (cmp_qual Conv) (cmp_universe2 Conv) u2 u2' -> cmp_instance (cmp_qual Conv) (cmp_universe3 Conv) u3 u3') ->
  ...
  cmp_instance (cmp_qual Conv) (cmp_universe1 Conv) u1 u1' \/ cmp_universe_instance_variance cmp_universe1 pb1 v (Instance.universes u1) (Instance.universes u1') ->
  cmp_instance (cmp_qual Conv) (cmp_universe2 Conv) u2 u2' \/ cmp_universe_instance_variance cmp_universe2 pb2 v (Instance.universes u2) (Instance.universes u2') ->
  cmp_instance (cmp_qual Conv) (cmp_universe3 Conv) u3 u3' \/ cmp_universe_instance_variance cmp_universe3 pb3 v (Instance.universes u3) (Instance.universes u3').
Proof.
  intros Hsub1 Hsub2 Hl Hr e [H|H] [H'|H']; [left; apply Hl| right; apply Hr ..]; auto.
  all: apply (cmp_instance_variance cmp_qual); tas.
  - rewrite e; eapply Forall2_length.
    + destruct H as [_ Hu]; eauto.
    + eapply @Forall3_Forall2_right with (Q := fun _ _ => True); eauto.
    + eapply @Forall3_Forall2_left with (Q := fun _ _ => True); eauto.
  - rewrite -e.
    all: eapply Forall2_length.
    + eapply @Forall3_Forall2_right with (Q := fun _ _ => True); eauto.
    + destruct H' as [_ Hu]; eauto.
    + eapply @Forall3_Forall2_left with (Q := fun _ _ => True); eauto.
Qed.
...
#[global]
Instance cmp_universe_instance_refl cmp_quality cmp_universe :
...
#[global]
Instance cmp_universe_instance_sym cmp_quality cmp_universe :
...
#[global]
Instance cmp_universe_instance_trans cmp_quality cmp_universe :
...
Lemma cmp_universe_instance_eq {u u'} : cmp_instance eq eq u u' -> u = u'.
Proof.
  intros H. unfold cmp_instance, on_rel in H.
  destruct H as [Hq Hu].
  apply Forall2_map, Forall2_eq, map_inj in Hq, Hu; revgoals.
  tauto. intros ?? e. now inv e.
...
Lemma cmp_universe_instance_flip eq_quality eq_quality' eq_universe eq_universe' u u' :
