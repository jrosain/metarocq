...
Section UniverseClosedSubst.
  ...
  Lemma closedq_quality_leq q n k : k <= n -> closedq_quality k q -> closedq_quality n q.
  Proof.
    destruct q; cbnr. destruct t. intro hle.
    destruct k eqn:ek; auto. destruct n eqn:en; auto.
    - inversion hle.
    - intro hleb. apply (ssrbool.elimT (Nat.leb_spec0 n0 n1)) in hleb.
      apply (ssrbool.introT (Nat.leb_spec0 n0 n2)); lia.
  Qed.                            

  Lemma closedu_level_leq l n k : k <= n -> closedu_level k l -> closedu_level n l.
  Proof.
    destruct l; cbnr. intros hle.
    destruct k eqn:ek; auto.
    destruct n eqn:en; auto.
    - inversion hle.
    - intro hleb. apply (ssrbool.elimT (Nat.leb_spec0 n0 n1)) in hleb.
      apply (ssrbool.introT (Nat.leb_spec0 n0 n2)); lia.
  Qed.

  Lemma closedu_universe_leq u n k : k <= n -> closedu_universe k u -> closedu_universe n u.
  Proof.
    intro hle. unfold closedu_universe, closedu_level_expr.
    rewrite !for_all_elements -!forallb_Forall. intros H.
    eapply Forall_impl. 1: eauto.
    intros []. cbn. now apply closedu_level_leq.
  Qed.

  Lemma closedu_sort_leq s n k : k <= n -> closedu_sort k s -> closedu_sort n s.
  Proof.
    destruct s; cbnr. apply closedu_universe_leq.
  Qed.
...
Section SubstInstanceClosed.
  ...
  Proof using Hcl.
    ...
    - intros _. eapply forallb_forall in H; tea.
    - intros H'. eapply forallb_nth in H. destruct H as [q [e' H]].
