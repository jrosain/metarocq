...
Module Universe.
  ...
  Definition sup_ {ulvl} univ_sup (u u' : t_ Sort.t ulvl) : t_ Sort.t ulvl :=
    match u.(s), u'.(s) with
    | sSProp, _ => mk u'.(s) (univ_sup u.(l) u'.(l))
    | sProp, sSProp => mk sProp (univ_sup u.(l) u'.(l))
    | sProp, _ => mk u'.(s) (univ_sup u.(l) u'.(l))
    | sType, _ => mk sType (univ_sup u.(l) u'.(l))
    end.
  Definition sup : t -> t -> t := sup_ UnivLvl.sup.
  Definition csup := sup_ Nat.max.

  (** Type of a product *)
  Definition sort_of_product_ {ulvl} usup (dom codom : t_ Sort.t ulvl) : t_ Sort.t ulvl :=
    match codom.(s) with
    | sSProp | sProp => codom
    (* Prop and SProp impredicativity *)
    | _ => sup_ usup dom codom
    end.
  Definition sort_of_product : t -> t -> t := sort_of_product_ UnivLvl.sup.
...
Lemma is_prop_sort_prod u1 u2 :
  Universe.is_prop (Universe.sort_of_product u1 u2)
  -> Universe.is_prop u2.
Proof.
...
Lemma is_sprop_sort_prod u1 u2 :
  Universe.is_sprop (Universe.sort_of_product u1 u2)
...
Section UnivCompare.
  ...
  Definition leq_univ_n n φ := leq_univ_n_ (fun n => leq_universe_n n φ) n.
  ...
  Definition eq_univ φ := eq_univ_ (eq_universe φ).
  ...
  Lemma compare_univ_type φ pb l l' :
    compare_univ φ pb (Universe.mk sType l) (Universe.mk sType l') = compare_universe φ pb l l'.
  ...
  (** u + n <= u' *)
  Definition leq_csort_n : Z -> concrete_univ -> concrete_univ -> Prop :=
    leq_univ_n_ (fun n u u' => (Z.of_nat u <= Z.of_nat u' - n)%Z).

  Definition leq_csort := leq_univ_n 0.
  Definition lt_csort := leq_univ_n 1.
  ...
  Lemma csort_sup_comm u u' : Universe.csup u u' = Universe.csup u' u.
  Proof using Type.
    destruct u as [[| |] n], u' as [[| |] m] => //=; cbnr; f_equal; lia.
  Qed.

  Lemma csort_sup_not_uproplevel u u' :
  ...
  Lemma csort_sup_mon u0 u' l l' : (u0 <= u')%u -> (Universe.mk sType l <= Universe.mk sType l')%u ->
    (Universe.csup u0 (Universe.mk sType l) <= Universe.csup u' (Universe.mk sType u'))%u.
  Proof using Type.
  ...
  Lemma leq_csort_of_product_mon u u' v v' :
    (u <= u')%u ->
    (v <= v')%u ->
    (Universe.csort_of_product u v <= Universe.csort_of_product u' v')%u.
  Proof using Type.
    intros Hle1 Hle2.
  ...
  Lemma impredicative_csort_product {univ} {univ_sup} {l u} :
    ...
    Universe.sort_of_product_ (univ := univ) univ_sup l u = u.
  Proof using Type. now destruct u. Qed.

  Lemma leq_sort_sup_l φ u1 s2 :
    let s1 := sType u1 in
    leq_sort φ s1 (Universe.sup s1 s2).
  Proof using Type.
  ...
  Lemma leq_sort_sup_r φ s1 u2 :
    let s2 := sType u2 in
    leq_sort φ s2 (Universe.sup s1 s2).
  Proof using Type.
    destruct s1 as [| | u1]; cbnr.
    apply leq_universe_sup_r.
  Qed.

  Lemma leq_sort_product φ (s1 s2 : Universe.t)
    : leq_sort φ s2 (Universe.sort_of_product s1 s2).
  Proof using Type.
    destruct s2 as [| | u2] => //.
...
Definition relevance_of_family (s : Universe.family) :=
  match s with
  | Universe.fSProp => Irrelevant
  | _ => Relevant
  end.

#[global] Opaque relevance_of_family.

Notation rel_of_Type := (relevance_of_family Universe.fType).
Notation relevance_of_sort s := (relevance_of_family (Universe.to_family s)).
...
Definition is_lSet {cf} φ s := eq_sort φ s Universe.type0.
  (* Unfolded definition :
  match s with
  | Universe.sType u =>
...
Lemma relevance_super s : relevance_of_sort (Universe.super s) = rel_of_Type.
Proof using Type.
  now destruct s.
Qed.
...
Lemma leq_sort_product_mon {cf} ϕ s1 s1' s2 s2' :
  leq_sort ϕ s1 s1' ->
  leq_sort ϕ s2 s2' ->
  leq_sort ϕ (Universe.sort_of_product s1 s2) (Universe.sort_of_product s1' s2').
Proof.
...
Lemma impredicative_product {cf} {ϕ l u} :
  Universe.is_propositional u ->
  leq_sort ϕ (Universe.sort_of_product l u) u.
Proof.
