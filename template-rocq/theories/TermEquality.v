...
Definition cmp_instance (cmp_qual : Quality.t -> Quality.t -> Prop) (cmp_univ : Universe.t -> Universe.t -> Prop) : Instance.t -> Instance.t -> Prop :=
...
Definition cmp_universe_instance_variance cmp_univ pb v u u' :=
  Forall3 (cmp_universe_variance cmp_univ pb) v (Instance.universes u) (Instance.universes u').
...
Definition cmp_opt_variance cmp_qual cmp_univ pb v :=
  ...
  | AllIrrelevant => fun u u' => #|Instance.qualities u| = #|Instance.qualities u'| /\
                              #|Instance.universes u| = #|Instance.universes u'|
  | Variance v => fun u u' => cmp_instance (cmp_qual Conv) (cmp_univ Conv) u u' \/
                            (cmp_quality_instance (cmp_qual Conv) u u' /\
                               cmp_universe_instance_variance cmp_univ pb v u u')
  end.
...
Proof.
  intros u u' H.
  destruct v as [| |v] => //=.
  unfold cmp_opt_variance, cmp_instance in H.
...
Lemma cmp_instance_variance cmp_qual cmp_univ pb v u u' :
  ...
  cmp_instance (cmp_qual Conv) (cmp_univ Conv) u u' -> cmp_universe_instance_variance cmp_univ pb v u u'.
Proof.
  intros Hsub Hlen Hu.
  apply Forall2_triv in Hlen.
  unfold cmp_instance in Hu. destruct Hu as (Hq & Hu).
...
Lemma cmp_opt_variance_var_dec cmp_qual cmp_univ pb v ui ui' :
  RelationClasses.subrelation (cmp_univ Conv) (cmp_univ pb) ->
  cmp_instance (cmp_qual Conv) (cmp_univ Conv) ui ui' \/ (cmp_quality_instance (cmp_qual Conv) ui ui' /\ cmp_universe_instance_variance cmp_univ pb v ui ui') ->
  { cmp_instance (cmp_qual Conv) (cmp_univ Conv) ui ui' } + { cmp_quality_instance (cmp_qual Conv) ui ui' /\ cmp_universe_instance_variance cmp_univ pb v ui ui' }.
Proof.
  intros subr H.
...
Definition cmp_global_instance_gen Σ cmp_quality cmp_universe pb gr napp :=
  cmp_opt_variance cmp_quality cmp_universe pb (global_variance_gen Σ gr napp).
...
Proof.
  intros H x y xy. unfold cmp_quality_instance in *.
...
Proof.
  intros H x y xy. unfold cmp_universe_instance in *.
  eapply Forall2_impl; tea; unfold on_rel; auto.
Qed.
...
Proof.
  intros H H' x y xy. unfold cmp_instance in *.
  destruct xy as (xyq & xyu). split; eapply Forall2_impl; tea; unfold on_rel; auto.
Qed.

Lemma cmp_universe_instance_impl' Q Q' R R' :
  RelationClasses.subrelation Q Q' ->
  RelationClasses.subrelation R R' ->
...
Inductive eq_term_upto_univ_napp Σ
  (cmp_qual : conv_pb -> Quality.t -> Quality.t -> Prop)                                 
  (cmp_universe : conv_pb -> Universe.t -> Universe.t -> Prop)
  (cmp_sort : conv_pb -> sort -> sort -> Prop)
  (pb : conv_pb) (napp : nat) : term -> term -> Type :=
| eq_Rel n  :
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tRel n) (tRel n)

| eq_Evar e args args' :
    All2 (eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0) args args' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tEvar e args) (tEvar e args')

| eq_Var id :
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tVar id) (tVar id)

| eq_Sort s s' :
    cmp_sort pb s s' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tSort s) (tSort s')

| eq_App t t' u u' :
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb (#|u| + napp) t t' ->
    All2 (eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0) u u' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tApp t u) (tApp t' u')

| eq_Const c u u' :
    cmp_instance (cmp_qual Conv) (cmp_universe Conv) u u' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tConst c u) (tConst c u')

| eq_Ind i u u' :
    cmp_global_instance Σ cmp_qual cmp_universe pb (IndRef i) napp u u' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tInd i u) (tInd i u')

| eq_Construct i k u u' :
    cmp_global_instance Σ cmp_qual cmp_universe pb (ConstructRef i k) napp u u' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tConstruct i k u) (tConstruct i k u')

| eq_Lambda na na' ty ty' t t' :
    eq_binder_annot na na' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 ty ty' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 t t' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tLambda na ty t) (tLambda na' ty' t')

| eq_Prod na na' a a' b b' :
    eq_binder_annot na na' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 a a' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb 0 b b' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tProd na a b) (tProd na' a' b')

| eq_LetIn na na' t t' ty ty' u u' :
    eq_binder_annot na na' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 t t' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 ty ty' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 u u' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tLetIn na t ty u) (tLetIn na' t' ty' u')

| eq_Case ind p p' c c' brs brs' :
    All2 (eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0) p.(pparams) p'.(pparams) ->
    cmp_instance (cmp_qual Conv) (cmp_universe Conv) p.(puinst) p'.(puinst) ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 p.(preturn) p'.(preturn) ->
    All2 eq_binder_annot p.(pcontext) p'.(pcontext) ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 c c' ->
    All2 (fun x y =>
      All2 (eq_binder_annot) (bcontext x) (bcontext y) *
      eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 (bbody x) (bbody y)
    ) brs brs' ->
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tCase ind p c brs) (tCase ind p' c' brs')

| eq_Proj p c c' :
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 c c' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tProj p c) (tProj p c')

| eq_Fix mfix mfix' idx :
    All2 (fun x y =>
      eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 x.(dtype) y.(dtype) *
      eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 x.(dbody) y.(dbody) *
      (x.(rarg) = y.(rarg)) *
      eq_binder_annot x.(dname) y.(dname)
    )%type mfix mfix' ->
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tFix mfix idx) (tFix mfix' idx)
...
| eq_CoFix mfix mfix' idx :
    ...
    eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tCoFix mfix idx) (tCoFix mfix' idx)

| eq_Cast t1 c t2 t1' c' t2' :
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 t1 t1' ->
  eq_cast_kind c c' ->
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 t2 t2' ->
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tCast t1 c t2) (tCast t1' c' t2')

| eq_Int i : eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tInt i) (tInt i)
| eq_Float f : eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tFloat f) (tFloat f)
| eq_String s : eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tString s) (tString s)
| eq_Array u u' arr arr' def def' ty ty' :
  cmp_instance (cmp_qual Conv) (cmp_universe Conv) (Instance.make [] [u]) (Instance.make [] [u']) ->
  All2 (eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0) arr arr' ->
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 def def' ->
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv 0 ty ty' ->
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (tArray u arr def ty) (tArray u' arr' def' ty').

Notation eq_term_upto_univ Σ cmp_qual cmp_universe cmp_sort pb := (eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb 0).

(* ** Syntactic conversion/cumulativity up-to universes *)

Definition compare_term `{checker_flags} Σ φ (pb : conv_pb) :=
  eq_term_upto_univ Σ compare_quality (compare_universe φ) (compare_sort φ) pb.

Notation eq_term Σ φ := (compare_term Σ φ Conv).
Notation leq_term Σ φ := (compare_term Σ φ Cumul).

Lemma cmp_instance_refl cmp_qual cmp_universe :
  RelationClasses.Reflexive cmp_universe ->
  RelationClasses.Reflexive cmp_qual ->
  forall u, cmp_instance cmp_qual cmp_universe u u.
Proof.
  intros refl_univ refl_qual u.
  unfold cmp_instance; split; apply Forall2_same; reflexivity.
Qed.

Lemma cmp_global_instance_refl Σ cmp_qual cmp_universe pb gr napp :
  ...
  forall u, cmp_global_instance Σ cmp_qual cmp_universe pb gr napp u u.
Proof.
  intros rRE rRQ rRle.
  unfold cmp_global_instance_gen.
  destruct global_variance_gen as [| |v] => //= u.
  - now apply cmp_instance_refl.
  - left. now apply cmp_instance_refl.
Qed.
...
Lemma eq_term_upto_univ_refl Σ cmp_qual cmp_universe cmp_sort pb :
  RelationClasses.Reflexive (cmp_qual Conv) ->
  RelationClasses.Reflexive (cmp_universe Conv) ->
  RelationClasses.Reflexive (cmp_universe pb) ->
  RelationClasses.Reflexive (cmp_sort Conv) ->
  RelationClasses.Reflexive (cmp_sort pb) ->
  forall napp t, eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp t t.
Proof.
  intros refl_qual refl_univ refl_univ' refl_sort refl_sort' napp t.
...
#[global] Instance cmp_global_instance_impl_same_napp Σ cmp_qual cmp_qual' cmp_universe cmp_universe' pb pb' gr napp :
  RelationClasses.subrelation (cmp_qual Conv) (cmp_qual' Conv) ->
  ...
  subrelation (cmp_global_instance Σ cmp_qual cmp_universe pb gr napp) (cmp_global_instance Σ cmp_qual' cmp_universe' pb' gr napp).
Proof.
  ...
  intros [H | [Hq Hu]]; [left | right].
  1: eapply cmp_instance_impl; tea.

  split.
  1: eapply cmp_quality_instance_impl; tea.
  eapply Forall3_impl; tea. clear Hu v u u' Hq.
  intros v u u'.
  destruct v => //=.
  all: unfold on_rel; now auto.
Qed.
...
#[global]
Instance cmp_global_instance_impl 
  Σ cmp_qual cmp_qual' cmp_universe cmp_universe' pb pb' gr napp napp' :
  RelationClasses.subrelation (cmp_qual Conv) (cmp_qual' Conv) ->
  RelationClasses.subrelation (cmp_universe Conv) (cmp_universe' Conv) ->
  RelationClasses.subrelation (cmp_universe pb) (cmp_universe' pb') ->
  napp <= napp' ->
...
Proof.
  ...
  - intros [H | [Hq Hu]]; [left | right].
    1: eapply cmp_instance_impl; tea.
    split.
    * eapply cmp_quality_instance_impl; eassumption.
    * eapply cmp_universe_instance_variance_impl; eassumption.
Qed.

#[global] Instance eq_term_upto_univ_impl 
  Σ cmp_qual cmp_qual' cmp_universe cmp_universe' cmp_sort cmp_sort' pb pb' napp napp' :
  RelationClasses.subrelation (cmp_qual Conv) (cmp_qual' Conv) ->
  RelationClasses.subrelation (cmp_universe Conv) (cmp_universe' Conv) ->
  RelationClasses.subrelation (cmp_universe pb) (cmp_universe' pb') ->
  ...
  napp <= napp' ->
  subrelation (eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp) (eq_term_upto_univ_napp Σ cmp_qual' cmp_universe' cmp_sort' pb' napp').
Proof.
  ...
  - inversion 1; subst; constructor.
    ...
    solve_all.
  - inversion 1; subst; constructor.
    eapply cmp_global_instance_impl; eauto.
  - inversion 1; subst; constructor.
    eapply cmp_global_instance_impl; eauto.
  ...
  - inversion 1; subst; constructor.
    ...
    cbn. intros x [? ?] y [[[? ?] ?] ?]. repeat split; eauto.
  - inversion 1; subst; constructor; eauto.
    1: eapply cmp_instance_impl; eauto.
    eapply All2_impl'; tea. eapply All_impl; eauto.
Qed.

Lemma eq_term_upto_univ_morphism0 
  Σ cmp_qual cmp_qual' cmp_universe cmp_universe' cmp_sort cmp_sort' :
  RelationClasses.subrelation (cmp_qual Conv) (cmp_qual' Conv) ->
  RelationClasses.subrelation (cmp_universe Conv) (cmp_universe' Conv) ->
  RelationClasses.subrelation (cmp_sort Conv) (cmp_sort' Conv) ->
  forall t u napp, eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort Conv napp t u -> eq_term_upto_univ_napp Σ cmp_qual' cmp_universe' cmp_sort' Conv napp t u.
Proof.
  intros qual_sub univ_sub sort_sub t u napp.
  apply eq_term_upto_univ_impl.
  5: auto with arith. all: auto.
Qed.

Lemma eq_term_upto_univ_morphism 
  Σ cmp_qual cmp_qual' cmp_universe cmp_universe' cmp_sort cmp_sort' pb pb' :
  RelationClasses.subrelation (cmp_qual Conv) (cmp_qual' Conv) ->
  RelationClasses.subrelation (cmp_universe Conv) (cmp_universe' Conv) ->
  RelationClasses.subrelation (cmp_universe pb) (cmp_universe' pb') ->
  RelationClasses.subrelation (cmp_sort Conv) (cmp_sort' Conv) ->
  RelationClasses.subrelation (cmp_sort pb) (cmp_sort' pb') ->
  forall t u napp, eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp t u -> eq_term_upto_univ_napp Σ cmp_qual' cmp_universe' cmp_sort' pb' napp t u.
Proof.
  intros qual_sub univ_sub univ_sub_pb sort_sub sort_sub_pb t u napp.
  apply eq_term_upto_univ_impl.
  5: auto with arith. all: auto.
Qed.
...
Lemma eq_term_upto_univ_App `{checker_flags} Σ cmp_qual cmp_universe cmp_sort pb napp f f' :
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp f f' ->
  isApp f = isApp f'.
Proof.
...
Lemma eq_term_upto_univ_mkApps `{checker_flags} Σ cmp_qual cmp_universe cmp_sort pb napp f l f' l' :
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb (#|l| + napp) f f' ->
  All2 (eq_term_upto_univ Σ cmp_qual cmp_universe cmp_sort Conv) l l' ->
  eq_term_upto_univ_napp Σ cmp_qual cmp_universe cmp_sort pb napp (mkApps f l) (mkApps f' l').
Proof.
  induction l in f, f' |- *; intro e; inversion_clear 1.
  - assumption.
  - pose proof (eq_term_upto_univ_App _ _ _ _ _ _ _ _ e).
    case_eq (isApp f).
...
Proof.
  ...
  eapply eq_term_upto_univ_impl. 7:eauto. 6:auto with arith.
