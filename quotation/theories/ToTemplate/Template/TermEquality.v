From MetaRocq.Template Require Import Ast TermEquality.
From MetaRocq.Quotation.ToTemplate Require Import Init.
From MetaRocq.Quotation.ToTemplate Require Import (hints) Stdlib.Init Stdlib.Lists Stdlib.Numbers Stdlib.Floats.
From MetaRocq.Quotation.ToTemplate.Utils Require Import (hints) utils All_Forall.
...
Section with_R.
  Context {cmp_quality : conv_pb -> Quality.t -> Quality.t -> Prop}
          {cmp_universe : conv_pb -> Universe.t -> Universe.t -> Prop}
          {qRe : quotation_of cmp_universe} {qRq : quotation_of cmp_quality}
  ...
  Proof using cmp_universe qRq qRe quoteRq quoteRe.
...
#[export] Instance quote_eq_term_upto_univ_napp
 ...
 {qRq : quotation_of cmp_quality} {qRe : quotation_of cmp_universe} {qRle : quotation_of cmp_sort}
 {quoteRq : forall pb x y, ground_quotable (cmp_quality pb x y)} {quoteRe : forall pb x y, ground_quotable (cmp_universe pb x y)} {quoteRle : forall pb x y, ground_quotable (cmp_sort pb x y)}
 {Σ pb napp x y}
